...
var _default = exports.default = (0, _utils2.createRule)({
  ...
  create(context, [{
    ...
    const maybeResetCount = node => {
      const isTestFn = node.parent && node.parent.type !== _utils.AST_NODE_TYPES.CallExpression || (0, _utils2.isTypeOfJestFnCall)(node.parent, context, ['test']);
      if (isTestFn) {
        count = 0;
      }
    };
    return {
      ...
      CallExpression(node) {
        ...
        if (jestFnCall && jestFnCall.type === 'test') {
          count = 0;
        }
        if (jestFnCall && jestFnCall.type !== 'expect' || jestFnCall.head.node.parent && jestFnCall.head.node.parent.type === _utils.AST_NODE_TYPES.MemberExpression) {
          return;
        }
        count += 1;
        if (count > max) {
